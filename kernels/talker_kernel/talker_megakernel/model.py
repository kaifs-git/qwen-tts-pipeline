...
def load_weights(model_name="Qwen/Qwen3-TTS", verbose: bool = True):
    ...
    NOTE: weight-key mapping below assumes the upstream Qwen3-0.6B state_dict
    layout (`model.layers.{i}.self_attn.q_proj.weight`, etc.). The Qwen3-TTS
    talker uses `talker.model.layers.{i}.*` keys — the loader is updated in a
    follow-up commit (B1.2). This stub keeps the original 0.6B path for build
    smoke-testing only; it cannot produce correct talker outputs as-is.
    """
    if not verbose:
        import os
